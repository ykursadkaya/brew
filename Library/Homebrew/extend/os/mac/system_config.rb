class SystemConfig
  class << self
    ...
    def describe_java
      ...
      out, _, status = system_command("/usr/libexec/java_home", args: ["--xml", "--failfast"])
      return "N/A" unless status.success?
      javas = []
      REXML::XPath.each(REXML::Document.new(out), "//key[text()='JVMVersion']/following-sibling::string") do |item|
        javas << item.text
      end
      javas.uniq.join(", ")
    end
